#!/usr/bin/env python3
"""
EcoLab Smart Socket Simulator (Python) - Socket 5
...
MQTT_PORT = 8883
MQTT_USERNAME = "smartsocket5"
MQTT_PASSWORD = "smart5"
CA_CERT = os.path.join(os.path.dirname(os.path.dirname(__file__)), "credentials", "ca.crt")

TOPIC_CONTROL = "ecolab/socket/5/control"
TOPIC_ENERGY = "ecolab/socket/5/energy"
TOPIC_DEVICE_STATUS = "ecolab/socket/5/devicestatus"
TOPIC_RELAY_STATUS = "ecolab/socket/5/relaystatus"
TOPIC_TIMER = "ecolab/socket/5/timer"
TOPIC_TIMER_STATUS = "ecolab/socket/5/timer/status"
TOPIC_SCHEDULE_START = "ecolab/socket/5/schedule/start"
TOPIC_SCHEDULE_STOP = "ecolab/socket/5/schedule/stop"
TOPIC_SCHEDULE_MODE = "ecolab/socket/5/schedule/mode"
TOPIC_SCHEDULE_STATUS = "ecolab/socket/5/schedule/status"
TOPIC_DATETIME_STATUS = "ecolab/socket/5/datetime/status"
...
running = True
SCHEDULE_FILE = "schedule_socket5.json"
...
def restore_relay_state():
    ...
    if schedule_start_active and schedule_stop_active:
        relay_state = is_within_scheduled_on_window(datetime.now())
    else:
...
def publish_status_sync():
    send_relay_status()
    if timer_active:
        remaining = max(0, timer_duration - (current_millis() - timer_start_millis))
    ...
    now = datetime.now()
    client.publish(TOPIC_DATETIME_STATUS, f"OK:NTP_SYNCED:{now.strftime('%Y-%m-%d %H:%M:%S')} {now.isoweekday()}", retain=True)
...
def on_connect(client, userdata, flags, rc):
    ...
    mqtt_connected = rc == 0
    if mqtt_connected:
        for topic in (TOPIC_CONTROL, TOPIC_TIMER, TOPIC_SCHEDULE_START, TOPIC_SCHEDULE_STOP, TOPIC_SCHEDULE_MODE):
...
def send_energy_data():
    ...
    if relay_state:
        ...
        frequency = 0.0
        pf = 0.0
    client.publish(TOPIC_ENERGY, json.dumps({"voltage": voltage, "current": current, "power": power, "energy": energy, "frequency": frequency, "pf": pf}))
...
def input_thread():
    ...
    while running:
        try:
            user_input = input("\n>> Masukkan beban Socket 5 (A): ").strip()
...
def simulate_leds():
    print(f"\r[SOCKET 5] MQTT: {'OK' if mqtt_connected else 'OFF'} | RELAY: {'ON' if relay_state else 'OFF'}    ", end="", flush=True)


def main():
    global client, running
    client = mqtt.Client(client_id="smartsocket5_simulator")
